    ...
    T: VarIntAsyncWriter + AsyncWrite + Unpin + Send,
{
    ...
    async fn write_field_header(&mut self, field_type: u8, field_id: i16) -> Result<()> {
    ...
    T: VarIntAsyncWriter + AsyncWrite + Unpin + Send,
{
    ...
    async fn write_struct_begin(&mut self, _: &TStructIdentifier) -> Result<()> {
    ...
    async fn write_byte(&mut self, b: u8) -> Result<()> {
        self.transport
            .write(&[b])
            .await
            .map_err(From::from)
